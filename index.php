...
<div>
 ...
 <a href="post.php?id=<?=$post['id']?>" style="text-decoration:None;color:black">
   ...
   <?php include_once('include/sidebar.php');?>
    ...
    echo $total_pages=ceil($total_posts/$post_per_page);
    ?>
    <nav aria-label="Page navigation example">
        ...
        ?>
          ...
          <li class="page-item <?=$switch?>">
            <a class="page-link"  href="?<?php if(isset($_GET['serach'])){echo "search=$keywoed&";}?>page=<?=$page-1?>" tabindex="-1" aria-disabled="true">Previous</a>
          </li>
          <?php
            for($opage=1;$opage<=$total_pages;$opage++){
              ?>
                 <li class="page-item"><a class="page-link" href="?<?php if(isset($_GET['serach'])){echo "search=$keywoed&";}?>page=<?=$opage?>"><?=$opage?></a></li>
        ...
        ?>
          ...
          <li class="page-item <?=$nextswitch?>">
            <a class="page-link" href="?<?php if(isset($_GET['serach'])){echo "search=$keywoed&";}?>page=<?=$page+1?>">Next</a>
          </li>
        </ul>
      </nav>
